    ...
    <el-card class="bottom_card">
      <div v-show="scene == 0">
        ...
        <el-table style="margin:10px 0" border :data="attrArr">
          ...
          <el-table-column label="操作" width="150">
            <template #="{ row, $index }">
              <el-button type="warning" icon="Edit" size="small" @click="updateAttr"></el-button>
      ...
      <div v-show="scene == 1">
        ...
        <el-table :data="attrParams.attrValueList" style="width: 100%;margin: 10px 0;" border>
          ...
          <el-table-column label="属性值名称">
            <template #="{ row,$index }">
              <el-input placeholder="请输入属性值名称..." v-model="row.valueName" v-if="row.flag" @blur="toLook(row,$index)"/>
...
import { reqAttr, reqAddOrUpdateAttr,AttrValue } from '@/api/product/attr/index.ts'
