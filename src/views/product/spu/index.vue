    ...
    <el-card style="margin: 10px 0;">
      <div v-show="scene == 0">
        ...
        <el-table style="margin: 10px 0;" border :data="records">
          ...
          <el-table-column label="操作">
            <template #="{ row }">
              ...
              <el-button icon="Edit" type="warning" size="small" @click="updateSpu(row)" title="修改SPU"></el-button>
              <el-button icon="View" type="info" size="small" title="查看SKU列表" @click="findSku(row)"></el-button>
              <el-popconfirm :title="`你确定删除${row.spuName}吗?`" @confirm="deleteSpu(row)" width="200px">
                <template #reference>
                  <el-button icon="Delete" type="danger" size="small" title="删除SPU"></el-button>
                </template>
              </el-popconfirm>
            </template>
          </el-table-column>
        ...
        <el-pagination :page-count="totalPage" v-model:current-page="pageNo" v-model:page-size="pageSize"
          ...
          @current-change="getSpuData" @size-change="changeSize" prev-text="上一页" next-text="下一页" />
      </div>
...
import { ref, watch, computed,onBeforeUnMounted } from 'vue'
import useCategoryStore from '@/store/modules/category'
import { reqGetSPU, reqSkuList, reqDeleteSpu } from '@/api/product/spu/index.ts'
import type { HasSpuResponseData, Records, SpuData, SkuInfoData, SkuData } from '@/api/product/spu/types.ts'
